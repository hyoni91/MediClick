<div align= "center">
    <img src="https://capsule-render.vercel.app/api?type=waving&color=0:4556b0,100:a0ff52&height=180&text=MediClick&animation=&fontColor=ffffff&fontSize=70" />
    </div>
    

<h1>🩺 MediClick PROJECT</h1>
<br/>
<span>
MediClick은 암센터 웹 사이트를 구현한 프로젝트입니다.<br/>
환자와 관리자가 편하게 서비스를 이용할 수 있도록 설계했습니다.<br/?
    
본 프로젝트를 진행하며 Java, JavaScript, React, Maria DB, Spring, Mybatis 등<br/>
다양한 프로그래밍 언어뿐만 아니라 오픈API와 다양한 라이브러리 등을 활용하여<br/>
사용자에게 편리함을 줄 수 있는 웹 사이트를 만들기 위해 노력했습니다.</span>
<br/>


<h2>📌 목차</h2>

- [프로젝트 개요](#-프로젝트-개요)<br/>
- [팀원 소개](#-팀원-소개)
- [개발 환경](#-개발-환경)
- [프로젝트 수행 절차](#-프로젝트-수행-절차)
- [프로젝트 수행 결과](#-프로젝트-수행-결과)
- [프로젝트를 마치며](#-프로젝트를-마치며)

<h2>📝 프로젝트 개요</h2>
<h3>병원 실시간 진료 예약 기능 구현</h4>
<img width=650px src='https://github.com/user-attachments/assets/5ec8d3ac-4d3f-44cf-91ca-d9a735312528'><br />
- 관리자 & 환자 회원가입 및 로그인 기능 <br />
- 실시간 예약 기능 구현 및 예약 현황 조회 기능 <br />
<p/>

    
<h3>의료용 관리기기 온도 모니터링 시스템</h4>
<img width=650px src='https://github.com/user-attachments/assets/60ec53e3-0e41-4861-af25-b49acbbd873d'><br />

- 실시간 온도 측정 및 데이터베이스 자동 입력<br />

- 위험 온도를 경고음과 함께 경고 표시<br />

- 온도 데이터를 시각적으로 표현하기<br />
<p/>

    
<h3>공급사 관점 의료용품 수주 프로세스 구현</h4>
<img width=650px src='https://github.com/user-attachments/assets/6151f662-2668-4636-9578-5658000cdc3e'><br />
- 아이템 등록 및 발주 기능<br />
- 거래처 및 수주 관리<br />
- 재고 및 배송 관리
<p/>


<h2>👻팀원 소개</h2>

|[KHK](https://github.com/hyoni91)|[YJH](https://github.com/penguinnhi)|[SES](https://github.com/hotmnil5)|[MJH](https://github.com/ajw29k)|
|---|---|---|---|
|실시간 예약 기능|로그인 및 예약 상세페이지|환자 게시판|회원가입|
|대시보드 설계|라즈베리파이|카카오 api 길찾기 기능|Reactchart 차트 구현|
|거래처/수주관리 및 재고보조 |발주관리 및 수주보조|재고관리|아이템/배송 관리|



<h2>🔨 개발 환경</h2>
<!--
- **React** <img width=30px src='https://github.com/user-attachments/assets/bc1961c8-272b-4c4f-be80-a0cdac24364f'>
- **Node.js** <img width=30px src='https://github.com/user-attachments/assets/129e26ba-d580-482d-ad8c-d497d0f8672f'>
- **Java** <img width=30px src='https://github.com/user-attachments/assets/ef55fe03-c7a7-46db-8096-5a3858144649'>
- **Spring** <img width=30px margin=0px src='https://github.com/user-attachments/assets/2bc7d323-fa64-4d13-9060-9c56f48d5eae'>
- **MariaDB** <img width=30px src='https://github.com/user-attachments/assets/abaab39a-9f12-4085-b842-4018c3564b76'> 
- **MyBatis** <img width=60px src='https://github.com/user-attachments/assets/52dfc5c8-83ca-4689-bf49-3099e3084661'>
--!>
<div style="margin: ; text-align: left;" "text-align: left;"> <img src="https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=HTML5&logoColor=white">
          <img src="https://img.shields.io/badge/Java-007396?style=for-the-badge&logo=Java&logoColor=white">
          <img src="https://img.shields.io/badge/Javascript-F7DF1E?style=for-the-badge&logo=Javascript&logoColor=white">
          <img src="https://img.shields.io/badge/MariaDB-003545?style=for-the-badge&logo=MariaDB&logoColor=white">
          <img src="https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=MySQL&logoColor=white">
          <br/><img src="https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=Node.js&logoColor=white">
          <img src="https://img.shields.io/badge/React-61DAFB?style=for-the-badge&logo=React&logoColor=white">
          <img src="https://img.shields.io/badge/Spring Boot-6DB33F?style=for-the-badge&logo=Spring Boot&logoColor=white">
          </div>
    </div>

 **IDE/Editor** 
  - **Visual Studio Code**
  - **IntelliJ IDEA**
  - **Spring Boot**
  - **HeidiSQL**


<h2>✍ 프로젝트 수행 절차</h2>
<h4>병원 실시간 진료 예약 기능 구현</h4>
<h4>기간 : 2024.08.23 ~ 2024.09.06</h4>
- DataBase 작업 <br/>
- 메인 페이지 디자인 <br/>
- 회원가입 / 로그인 기능 구현 <br/>
- 의료진 / 진료과 페이지 구현 <br/>
- 게시판 작성, 수정, 삭제 기능구현 <br/>
- 실시간 예약 기능 구현 <br/>
- 예약 현황 조회 리스트 페이지 구현 <br/>
- 진료 현황 차트 기능 구현 <br/>

<h4>의료용 관리기기 온도 모니터링 시스템</h4>
<h4>기간 : 2024.09.09 ~ 2024.09.27</h4>
- DataBase 작업<br/>
- 라즈베리파이 작업<br/>
- DB insert / 연결<br/>
- 모니터링 페이지 디자인<br/>
- React 차트 기능 구현<br/>
- 실시간 온도 그래프 기능 구현<br/>
- 날씨 API 사용하여 기능 구현<br/>
- kako map API 사용 네비 / 길찾기 기능구현<br/>

<h4>공급사 관점 의료용품 수주 프로세스 구현</h4>
<h4>기간 : 2024.09.30~2024.10.23</h4>
- DataBase 작업<br/>
- 수주 발주 페이지 디자인<br/>
- 아이템 등록 기능 구현<br/>
- 거래처 관리 현황 기능구현<br/>
- 수주 / 발주 기능 구현<br/>
- MariaDB Trigger 작업<br/>
- 현 재고 현황 기능 구현<br/>
- 배송 현황 기능 구현<br/>


<h2>🧐 프로젝트 수행 결과</h2>
<h3>병원 실시간 진료 예약 기능 구현</h4>
<img width=650px src='https://github.com/user-attachments/assets/a4d75cb9-bf16-460e-b834-6d02511b8724'>
    
1. 환자/의료진 회원가입 및 로그인 기능

2. 실시간 예약 기능

3. 예약 조회 및 취소 기능

4. 과거 예약 자동 삭제

<h3>의료용 관리기기 온도 모니터링 시스템</h4>
<img width=650px src='https://github.com/user-attachments/assets/87a94024-bee5-45d8-862c-d43bd0c8442f'>


1. 실시간 온도 측정

2. 위험 온도를 넘어갈 시 부저 및 경고 알림
  
3. 실시간 온도를 바탕으로 그래프 자동 갱신 (10분마다 갱신)

4. 12시간 이후 온도 데이터 자동 삭제

5. 카카오맵 api 길찾기 기능 / 날씨 api  

<h3>공급사 관점 의료용품 수주 프로세스 구현</h4>

1. 거래처 관점 발주 기능 

2. 아이템 등록 및 트리고를 사용한 최초 재고 등록

3. 거래처 등록 및 수정 삭제 기능

4. 트리거를 사용한 수주 자동 등록

5. 출고시 재고 자동 반영


<h2>👏 프로젝트를 마치며</h2>

<h3>잘한점</h3>

- 카카오 맵, 날씨,  api를 사용해 볼 수 있어서 좋았다.

- 조인, group by, 트리거 등 다양한 쿼리를 사용해 볼 수 있었다.

- 리액트 캘린더, 리액트 모달 등의 라이브러리를 사용해 볼 수 있었다.


<h3>아쉬운점</h3>


- 실시간 예약 페이지의 디자인이 조금 아쉬웠다.

- 카카오 api를 두가지 이상을 사용하여 길찾기 기능을 구현했으나, 이 기능을 조금 더 적절한 곳에 쓰이도록 설계를 하지 못해 아쉬웠다.
 
